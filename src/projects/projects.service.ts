  ...
  private parseUserIds(userIds: any): number[] {
    if (typeof userIds === 'string') {
      try {
        ...
        if (!Array.isArray(parsed) || parsed.some(id => typeof id !== 'number')) {
          throw new Error();
        }
        return parsed;
      } catch {
        throw new BadRequestException('Invalid format for userIds. Must be JSON array of integers.');
      }
    } else if (Array.isArray(userIds) && userIds.every(id => typeof id === 'number')) {
      return userIds;
    } else if (!userIds) {
      return [];
    } else {
...
async create(createDto: CreateProjectDto, file?: Express.Multer.File) {
  ...
  if (notFoundIds.length) {
    throw new NotFoundException(`Users not found with IDs: ${notFoundIds.join(', ')}`);
  }
  ...
  async update(id: number, updateDto: UpdateProjectDto, file?: Express.Multer.File) {
  const project = await this.projectRepository.findOne({
    where: { id },
    relations: ['users'],
  });
  ...
  if (userIds.length > 0) {
    ...
    // ✅ Topilmagan user IDlarni aniqlash
    const foundIds = users.map(u => u.id);
    const notFoundIds = userIds.filter(id => !foundIds.includes(id));
  ...
  if (file && project.imagePath) {
    const oldImagePath = path.join(process.cwd(), 'uploads/projects', project.imagePath);
  ...
  return {
    ...
    data: {
      ...
      name: updated.name,
      description: updated.description,
      imagePath: updated.imagePath ? `uploads/projects/${updated.imagePath}` : null,
  ...
  async remove(id: number) {
    ...
    if (project.imagePath) {
      const filePath = path.join(process.cwd(), 'uploads/projects', project.imagePath);
